...
def count_reps(smoothed_angles):
    ...
    peaks, _ = find_peaks(smoothed_angles, height=PEAK_HEIGHT, distance=PEAK_DISTANCE, prominence=PEAK_PROMINENCE)
    return peaks


def find_bottom_frames(smoothed_angles):
    find_peaks = _find_peaks()
    valleys, _ = find_peaks(-smoothed_angles, distance=PEAK_DISTANCE, prominence=PEAK_PROMINENCE)
    return valleys
